...
app.get("/page", (req, res) => {
    ...
    con.query(`SELECT count(candidate_id) as count from candidate_details where is_delete = 0 ;`, (err, result) => {
        // console.log(result)
        page_no = Math.ceil(result[0].count / limit)
    });

    con.query(`select * from candidate_details where is_delete = 0  limit ${offset},${limit} `, (err, result) => {
        if (err) throw err;
        data = result;
        //console.log(data)
